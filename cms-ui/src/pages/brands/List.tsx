...
export const List: React.FC = () => {
  ...
  useEffect(() => { setLoading(true); http.get('/cms/brands').then(({data}) => setItems(data)).finally(() => setLoading(false)); }, []);
  const reload = () => http.get('/cms/brands').then(({data}) => setItems(data));
  const handleDelete = (id:string) => confirmAlert({ title:'Confirm Delete', message:'Are you sure want to delete?', buttons:[{ label:'Yes', className:'text-bg-danger', onClick:() => { setLoading(true); http.delete(`/cms/brands/${id}`).then(reload).finally(() => setLoading(false)); } }, { label:'No' }] });
