...
export const List: React.FC = () => {
    ...
    const getEffectivePaymentStatus = (order: any) => order.paymentMethod === 'eSewa' ? (order.payment?.status || order.paymentStatus || 'Pending') : (order.paymentStatus || order.payment?.status || 'Pending')
    const save = (orderId: string) => { http.patch(`/cms/orders/${orderId}`, editing[orderId] || {}).then(() => { setEditing(prev => ({...prev, [orderId]: {}})); load() }) }
    return loading ? <Loading /> : <Container><Row><Col className="page-card">
        ...
        <div className="d-flex flex-column gap-3">{filtered.map(order => {
            ...
            return <div className="order-admin-card" key={order._id}>
                <div className="d-flex justify-content-between flex-wrap gap-2 mb-3"><div><div className="fw-semibold">Order #{order._id.slice(-8).toUpperCase()}</div><div className="small text-muted">{order.user?.name} • {order.user?.email} • {dtFormat(order.createdAt)}</div></div><div className="d-flex gap-2 flex-wrap"><Badge bg={badge(order.status)}>{order.status}</Badge><Badge bg={badge(effectivePaymentStatus)}>{effectivePaymentStatus}</Badge><Badge bg="dark">{order.paymentMethod || 'COD'}</Badge></div></div>
                <div className="row g-3"><div className="col-lg-6"><div className="small text-muted mb-2">Items</div><div className="d-flex flex-column gap-2">{order.details?.map(item => <div className="order-line-admin" key={item._id}><img src={item.product?.images?.[0] ? imgUrl(item.product.images[0]) : '/vite.svg'} /><div className="flex-grow-1"><div className="fw-semibold">{item.product?.name}</div><div className="small text-muted">Qty {item.qty} × Rs. {item.price}</div></div><div className="fw-semibold">Rs. {item.total}</div></div>)}</div></div>
                    <div className="col-lg-6"><Row className="g-3"><Col md={6}><Form.Label>Order Status</Form.Label><Form.Select className="order-form-control text-center" value={patch.status ?? order.status} onChange={e => setEditing({...editing, [order._id]: {...patch, status: e.target.value as any}})}><option>Processing</option><option>Confirmed</option><option>Packed</option><option>Shipping</option><option>Delivered</option><option>Cancelled</option></Form.Select></Col><Col md={6}><Form.Label>Payment Status</Form.Label><Form.Select disabled={order.paymentMethod === 'eSewa'} className="order-form-control text-center" value={patch.paymentStatus ?? effectivePaymentStatus} onChange={e => setEditing({...editing, [order._id]: {...patch, paymentStatus: e.target.value as any}})}><option>Pending</option><option>Paid</option><option>Failed</option><option>Refunded</option></Form.Select></Col><Col md={6}><Form.Label>Tracking Code</Form.Label><Form.Control className="order-form-control text-center" value={patch.trackingCode ?? order.trackingCode ?? ''} onChange={e => setEditing({...editing, [order._id]: {...patch, trackingCode: e.target.value}})} placeholder="QC-TRACK-001" /></Col><Col md={6}><Form.Label>Payment Method</Form.Label><Form.Control className="order-form-control text-center" readOnly value={order.paymentMethod || 'COD'} /></Col><Col md={6}><Form.Label>Delivery Phone</Form.Label><Form.Control className="order-form-control text-center" readOnly value={order.customerPhone || ''} /></Col><Col md={6}><Form.Label>Delivery Address</Form.Label><Form.Control className="order-form-control text-center" readOnly value={order.customerAddress || ''} /></Col><Col md={12}><Form.Label>Admin Note</Form.Label><Form.Control className="order-form-control" as="textarea" rows={2} value={patch.adminNote ?? order.adminNote ?? ''} onChange={e => setEditing({...editing, [order._id]: {...patch, adminNote: e.target.value}})} placeholder="Packing, courier handover, refund, etc." /></Col></Row><div className="d-flex justify-content-between align-items-center mt-3 flex-wrap gap-2"><div><div className="small text-muted">Order Amount</div><div className="fw-bold">Rs. {amount}</div></div><Button variant="dark" onClick={() => save(order._id)}>Save Update</Button></div></div></div>
            </div>
        })}</div>
    </Col></Row></Container>
}
